...
vec3 CalcPointLight(PointLight light, vec3 viewDir, vec3 norm);
...
void main() {
    ...
    for (int i = 0; i < POINT_LIGHTS_NR; i++) {
        result += CalcPointLight(pointLights[i], viewDir, norm);
    }
...
vec3 CalcPointLight(PointLight light, vec3 viewDir, vec3 norm) {
    vec3 normDir = normalize(FragPos - light.position);
    vec3 diffColor = texture(material.texture_diffuse1, TexPos).rgb;
...
vec3 CalcSpotLight(SpotLight light, vec3 viewDir, vec3 norm) {
    ...
    float theta = dot(spotDir, light.direction);
